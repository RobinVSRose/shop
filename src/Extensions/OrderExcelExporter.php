...
class OrderExcelExporter  extends AbstractExporter implements FromArray,ShouldAutoSize,WithStrictNullComparison,WithColumnFormatting
{
    use Exportable;
...
//        $query->getQueryBuilder()->getQuery()->cloneWithout(['orders']);//清除ID排序
        $query->select(['order.order_no','order.id','order.order_sn','order.transaction_id','order.member_id','user_bases.nickname',
